...
export const createRateLimitMiddleware = (): MiddlewareFn<Context> => {
    const rateLimitState = new Map<string | number, RateLimitEntry>();

    const getRateLimitKey = (ctx: Context): string | number | null => {
    ...
    const handleRateLimitedUpdate = async (ctx: Context): Promise<void> => {
